...
export class ContentfulController {
  ...
  static async createArticle(req: Request, res: Response) {
    try {
      ...
      if (
        articleData.publishDateScheduled &&
        typeof articleData.publishDateScheduled === "string"
      ) {
        articleData.publishDateScheduled = new Date(
          articleData.publishDateScheduled
        );
      }
      if (
        articleData.unpublishDateScheduled &&
        typeof articleData.unpublishDateScheduled === "string"
      ) {
        articleData.unpublishDateScheduled = new Date(
          articleData.unpublishDateScheduled
        );
      }
  ...
  static async updateArticle(req: Request, res: Response) {
    try {
      ...
      // Convert date strings to Date objects if provided
      if (
        updates.publishDateScheduled &&
        typeof updates.publishDateScheduled === "string"
      ) {
        updates.publishDateScheduled = new Date(updates.publishDateScheduled);
      }
      if (
        updates.unpublishDateScheduled &&
        typeof updates.unpublishDateScheduled === "string"
      ) {
        updates.unpublishDateScheduled = new Date(
          updates.unpublishDateScheduled
        );
      }
  ...
  static async uploadImage(req: Request, res: Response) {
    try {
      ...
      upload(req, res, async (err) => {
        ...
        try {
          // Call your service method with the correct parameters
