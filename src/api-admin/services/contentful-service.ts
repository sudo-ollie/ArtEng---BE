...
import { UploadImageParams } from "../../types/UploadImageParams";
...
export class ContentfulService {
  ...
  static async getArticleById(articleId: string): Promise<Article | null> {
    try {
      const response = await previewClient.getEntry(articleId, {
        include: 2,
      });
      ...
      if (article.fields.featuredImage?.sys?.linkType === "Asset") {
        try {
          const asset = await previewClient.getAsset(
  ...
  static async createArticle(articleData: ArticleFields) {
    try {
      ...
      if (articleData.publishDateScheduled) {
        fields.publishDateScheduled = {
          "en-US": articleData.publishDateScheduled.toISOString(),
        };
      }
      if (articleData.unpublishDateScheduled) {
        fields.unpublishDateScheduled = {
          "en-US": articleData.unpublishDateScheduled.toISOString(),
        };
      }
  ...
  static async uploadImage(params: UploadImageParams) {
    try {
      ...
      throw new Error("Asset processing timed out after 10 attempts");
    } catch (error) {
